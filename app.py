...
# Nastavenie stránky na centrovaný layout
    ...
    if st.button("🚀 ŠTART", use_container_width=True):
        if data_input:
            ...
            if temp_list:
                ...
                st.session_state.update({'picking_list': temp_list, 'current_index': 0, 'test_active': True, 'start_time': None, 'end_time': None, 'last_error': ""})
                st.rerun()
...
# --- 2. TEST ---
    ...
    # CENTROVANIE OBSAHU (Obrázok a text)
    st.write("") 
    
    # Použijeme stĺpce na vycentrovanie obrázka
    col_left, col_mid, col_right = st.columns([1, 2, 1])
    with col_mid:
        url = get_image_url(item['code'])
        st.image(url, use_container_width=True)

    # Centrovaný text pod obrázkom
    st.markdown(f"""
        <div style="text-align: center;">
            <h1 style="color: #3498db; margin-top: 0px; font-size: 2.5em;">{item['code']}</h1>
            <p style="font-size: 1.2em; opacity: 0.8;">EAN: {item['ean']}</p>
        </div>
